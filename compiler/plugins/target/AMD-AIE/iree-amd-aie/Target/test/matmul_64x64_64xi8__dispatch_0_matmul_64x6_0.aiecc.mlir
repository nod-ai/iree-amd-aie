...
aie.device(npu1_4col) {
  ...
  ^bb1:  // 2 preds: ^bb0, ^bb1
    ...
    aie.dma_bd(%buf2 : memref<8x16x4x8xi8>) {bd_id = 0 : i32, len = 4096 : i32, next_bd_id = 0 : i32}
  ...
  ^bb4:  // 2 preds: ^bb3, ^bb4
    ...
    aie.dma_bd(%buf1 : memref<8x8x8x8xi8>) {bd_id = 1 : i32, len = 4096 : i32, next_bd_id = 1 : i32}
  ...
  ^bb6:  // 2 preds: ^bb5, ^bb6
    ...
    aie.dma_bd(%buf0 : memref<8x16x4x8xi32>) {bd_id = 2 : i32, dimensions = #aie<bd_dim_layout_array[<size = 64, stride = 8>, <size = 8, stride = 512>, <size = 8, stride = 1>]>, len = 4096 : i32, next_bd_id = 2 : i32}
  ...
  ^bb1:  // 2 preds: ^bb0, ^bb1
    ...
    aie.dma_bd(%buf3 : memref<64x64xi32>) {bd_id = 0 : i32, len = 4096 : i32, next_bd_id = 0 : i32}
  ...
  ^bb4:  // 2 preds: ^bb3, ^bb4
    ...
    aie.dma_bd(%buf3 : memref<64x64xi32>) {bd_id = 1 : i32, len = 4096 : i32, next_bd_id = 1 : i32}
  ...
  ^bb1:  // 2 preds: ^bb0, ^bb1
    ...
    aie.dma_bd(%buf5 : memref<64x64xi8>) {bd_id = 0 : i32, len = 4096 : i32, next_bd_id = 0 : i32}
  ...
  ^bb4:  // 2 preds: ^bb3, ^bb4
    ...
    aie.dma_bd(%buf5 : memref<64x64xi8>) {bd_id = 1 : i32, dimensions = #aie<bd_dim_layout_array[<size = 8, stride = 8>, <size = 64, stride = 64>, <size = 8, stride = 1>]>, len = 4096 : i32, next_bd_id = 1 : i32}
  ...
  ^bb1:  // 2 preds: ^bb0, ^bb1
    ...
    aie.dma_bd(%buf4 : memref<64x64xi8>) {bd_id = 0 : i32, len = 4096 : i32, next_bd_id = 0 : i32}
  ...
  ^bb4:  // 2 preds: ^bb3, ^bb4
    ...
    aie.dma_bd(%buf4 : memref<64x64xi8>) {bd_id = 1 : i32, dimensions = #aie<bd_dim_layout_array[<size = 8, stride = 8>, <size = 64, stride = 64>, <size = 8, stride = 1>]>, len = 4096 : i32, next_bd_id = 1 : i32}
...
// CHECK: XAIE API: XAie_DmaSetAddrLen with args: &dmaDesc=ptr, basePlusOffsetInBytes=1024, lenInBytes=4096
...
// CHECK: XAie_DmaTensor(XAie_AieMlDmaDimDesc(StepSize: 1, Wrap: 8), XAie_AieMlDmaDimDesc(StepSize: 512, Wrap: 8), XAie_AieMlDmaDimDesc(StepSize: 8, Wrap: 64))
// CHECK: XAIE API: XAie_DmaSetMultiDimAddr with args: &dmaDesc=ptr, &dmaTileBdTensor=ptr, basePlusOffsetInBytes=9216, lenInBytes=16384
...
// CHECK: XAIE API: XAie_DmaChannelSetStartQueue with args: devInst=ptr, tileLoc=TileLoc(col: 0, row: 2), chNum=0, direction=1, bdId=2, repeatCount=1, enTokenIssue=0
...
// CHECK: XAIE API: dmaDesc.DmaMod->SetLock with args: &dmaDesc=ptr, acqLock=XAie_Lock(LockId: 65, LockVal: -1), relLock=XAie_Lock(LockId: 64, LockVal: 1), acqEn=1, relEn=0
// CHECK: XAIE API: XAie_DmaSetAddrLen with args: &dmaDesc=ptr, basePlusOffsetInBytes=524288, lenInBytes=4096
// CHECK: XAIE API: XAie_DmaSetNextBd with args: &dmaDesc=ptr, nextBdId.value()=0, enableNextBd=1
// CHECK: XAIE API: XAie_DmaEnableBd with args: &dmaDesc=ptr
// CHECK: XAIE API: XAie_DmaWriteBd with args: devInst=ptr, &dmaDesc=ptr, tileLoc=TileLoc(col: 0, row: 1), bdId=0
// CHECK: XAIE API: XAie_DmaDescInit with args: devInst=ptr, &dmaDesc=ptr, tileLoc=TileLoc(col: 0, row: 1)
// CHECK: XAIE API: dmaDesc.DmaMod->SetLock with args: &dmaDesc=ptr, acqLock=XAie_Lock(LockId: 64, LockVal: -1), relLock=XAie_Lock(LockId: 65, LockVal: 1), acqEn=1, relEn=0
// CHECK: XAie_DmaTensor(XAie_AieMlDmaDimDesc(StepSize: 1, Wrap: 2), XAie_AieMlDmaDimDesc(StepSize: 16, Wrap: 64), XAie_AieMlDmaDimDesc(StepSize: 2, Wrap: 8))
...
// CHECK: cdo-driver:     Address: 0x000000000021D000  Data is: 0x00400400
...
// CHECK: cdo-driver:     Address: 0x000000000021D020  Data is: 0x01400400
...
// CHECK: cdo-driver:     Address: 0x000000000021D040  Data is: 0x02401000
// CHECK: cdo-driver:     Address: 0x000000000021D044  Data is: 0x00000000
// CHECK: cdo-driver:     Address: 0x000000000021D048  Data is: 0x003FE000
// CHECK: cdo-driver:     Address: 0x000000000021D04C  Data is: 0x01010007
...
// CHECK: cdo-driver:     Address: 0x00000000001A0020  Data is: 0x00000400
...
// CHECK: cdo-driver:     Address: 0x00000000001A002C  Data is: 0x0080000F
...
// CHECK: cdo-driver:     Address: 0x00000000021A0000  Data is: 0x00000400
...
// CHECK: cdo-driver:     Address: 0x00000000021A0020  Data is: 0x00000400
...
// CHECK: cdo-driver:     Address: 0x00000000021A002C  Data is: 0x0080000F
