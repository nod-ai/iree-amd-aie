...
module {
  aie.device(npu1_1col) {
    ...
    ^bb1:  // pred: ^bb0
      ...
      aie.dma_bd(%objFifo_in0_cons_buff_0 : memref<16xi8>) {bd_id = 0 : i32, dimensions = #aie<bd_dim_layout_array[<size = 8, stride = 1>]>, len = 16 : i32, next_bd_id = 1 : i32, pad_dimensions = #aie<bd_pad_layout_array[<const_pad_before = 4, const_pad_after = 4>]>}
      aie.use_lock(%objFifo_in0_cons_prod_lock, Release, 1)
      aie.next_bd ^bb2
    ^bb2:  // pred: ^bb1
      aie.use_lock(%objFifo_in0_cons_cons_lock, AcquireGreaterEqual, 1)
      aie.dma_bd(%objFifo_in0_cons_buff_1 : memref<16xi8>) {bd_id = 1 : i32, dimensions = #aie<bd_dim_layout_array[<size = 8, stride = 1>]>, len = 16 : i32, pad_dimensions = #aie<bd_pad_layout_array[<const_pad_before = 4, const_pad_after = 4>]>}
      aie.use_lock(%objFifo_in0_cons_prod_lock, Release, 1)
      aie.next_bd ^bb3
    ^bb3:  // 2 preds: ^bb0, ^bb2
      aie.end
...
// CHECK: XAie_DmaTensor(XAie_AieMlDmaDimDesc(StepSize: 1, Wrap: 2))
// CHECK: XAIE API: XAie_DmaSetMultiDimAddr with args: &dmaDesc=ptr, &dmaTileBdTensor=ptr, basePlusOffsetInBytes=524288, lenInBytes=16
// CHECK: XAie_DmaPadTensor(XAie_PadDesc(Before: 1, After: 1))
...
// CHECK: XAIE API: dmaDesc.DmaMod->SetLock with args: &dmaDesc=ptr, acqLock=XAie_Lock(LockId: 65, LockVal: -1), relLock=XAie_Lock(LockId: 64, LockVal: 1), acqEn=1, relEn=0
// CHECK: XAie_DmaTensor(XAie_AieMlDmaDimDesc(StepSize: 1, Wrap: 2))
// CHECK: XAIE API: XAie_DmaSetMultiDimAddr with args: &dmaDesc=ptr, &dmaTileBdTensor=ptr, basePlusOffsetInBytes=524304, lenInBytes=16
// CHECK: XAie_DmaPadTensor(XAie_PadDesc(Before: 1, After: 1))
// CHECK: XAIE API: XAie_DmaSetPadding with args: &dmaDesc=ptr, &dmaPadTensor=ptr
// CHECK: XAIE API: XAie_DmaEnableBd with args: &dmaDesc=ptr
...
// CHECK: cdo-driver:     Address: 0x00000000001A0000  Data is: 0x00000004
// CHECK: cdo-driver:     Address: 0x00000000001A0004  Data is: 0x041A0000
// CHECK: cdo-driver:     Address: 0x00000000001A0008  Data is: 0x00040000
...
// CHECK: cdo-driver:     Address: 0x00000000001A0020  Data is: 0x00000004
// CHECK: cdo-driver:     Address: 0x00000000001A0024  Data is: 0x04020004
