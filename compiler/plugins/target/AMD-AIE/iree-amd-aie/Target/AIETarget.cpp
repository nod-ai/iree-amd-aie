...
LogicalResult AIETargetBackend::serializeExecutable(
  ...
  for (size_t i = 0; i < entryPointNames.size(); i++) {
    ...
    if (failed(aie2xclbin(
            ...
            /*outputCtrlPktInstPath=*/options.enableCtrlPkt
                ? std::make_optional(ctrlpktInstPath.str().str())
                : std::nullopt,
            /*outputCtrlPktSeqPath=*/options.enableCtrlPkt
  ...
  entryPointNameConvertor.squeeze();
  artifactConvertor.squeeze();
  asmInstrConverter.squeeze();
  reconfDataConverter.squeeze();
  switch (options.deviceHal) {
    case AMDAIEOptions::DeviceHAL::XRT: {
    ...
    case AMDAIEOptions::DeviceHAL::XRT_LITE: {
      ...
      llvm::errs() << "XRT_LITE\n";
      llvm::errs() << "Size of the first dimension: "
                   << asmInstrConverter.data.size() << "\n";
      for (size_t i = 0; i < asmInstrConverter.data.size(); ++i) {
        llvm::errs() << "Size of the second dimension "
                     << asmInstrConverter.data[i].size() << "\n";
        for (size_t j = 0; j < asmInstrConverter.data[i].size(); ++j) {
          llvm::errs() << "Size of the third dimension "
                       << asmInstrConverter.data[i][j].size() << "\n";
        }
      }

      llvm::errs() << "Size of the first dimension: "
                   << reconfDataConverter.data.size() << "\n";
      for (size_t i = 0; i < reconfDataConverter.data.size(); ++i) {
        llvm::errs() << "Size of the second dimension "
                     << reconfDataConverter.data[i].size() << "\n";
        for (size_t j = 0; j < reconfDataConverter.data[i].size(); ++j) {
          llvm::errs() << "Size of the third dimension "
                       << reconfDataConverter.data[i][j].size() << "\n";
        }
      }
