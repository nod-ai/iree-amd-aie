...
/// Obtain the loop count of `forOp` if all of start, end, and step
/// are constant, otherwise return std::nullopt. The loop count
/// is 'end - start / step'.
...
/// Starting at `op`, get the constant loop counts of all containing scf.for
/// ops, stopping at func.func. The returned vector is in reverse nesting
/// order, so for example if the function is called on 'my.op' in:
/// ```
...
/// }
/// ```
/// then the returned vector is (6, 4, 2).
///
/// If any loop count is not constant, return an nullopt.
...
/// Traverse recursively at most `n` times through parents of type
/// scf.for, starting at `op`, and return the final scf.for visited
/// if it exists. If there is no scf.for at depth `n`, return failure.
...
class Transformations {
 public:
  /// Factory constructor method that decodes a string description of the
  ...
  static FailureOr<Transformations> create(aievec::MatMulOp matmul,
    ...
    bool noUnrollAtEnd;
    if (failed(initialize(matmul, sequence, transformations, noUnrollAtEnd))) {
      return failure();
    }
    return Transformations(std::move(transformations), noUnrollAtEnd);
  }

  /// The total number of unrolls / unroll-and-jams to apply.
  uint32_t count() const { return transformations.size(); }

  /// The type of transformation to apply at step `n`.
  ...
  /// `n`. This depth is relative to the aievec.matmul operations(s) after
  ...
  /// The unrolling factor of the transformation to apply at step `n`.
 ...
 private:
  ...
  /// In the case where there is string 'sequence' pre-specifying what
  /// the sequence of transformations to do is, use a default strategy
  /// if one exists.
  ///
  /// Currently, if no default strategy exists, then `transformations`
  /// will remain empty, and `noUnrollAtEnd` will be false. This is
  /// equivalent to the pass doing nothing.
    ...
    if (isAie2(device)) {
      if (inType.isBF16()) {
        if (loopCounts == SmallVector<int>{4, 8, 8}) {
          // AKA:  "uj_0_2_uj_2_2_u_0_2_u_0_4_u_0_2_NOUNROLL";
  ...
  static LogicalResult initialize(
    ...
    if (splits.size() % 3 != 0) {
      return matmul->emitOpError()
             << errStart() << "that is not of size 3*n for some n.";
    }

    assert(splits.size() % 3 == 0);
    ...
    for (int i = 0; i < nTransformations; ++i) {
      ...
      if (!depth.has_value()) {
        return matmul->emitOpError()
               << errStart() << "with an invalid depth " << splits[i1] << '.';
      }
      std::optional<int> factor = detail::safeStoi(splits[i2]);
      if (!factor.has_value()) {
        return matmul->emitOpError()
               << errStart() << "with an invalid factor " << splits[i2] << '.';
      }
...
/// Apply the sequence of transformations described by `sequence` to the
/// scf.for operations that are ancestors of `rootMatMul`.
LogicalResult unrollJam(aievec::MatMulOp rootMatMul,
                        std::string_view sequence) {
  IRRewriter builder(rootMatMul->getContext());

  func::FuncOp funcOp = rootMatMul->getParentOfType<func::FuncOp>();
  ...
  FailureOr<Transformations> maybeTransformations =
      Transformations::create(rootMatMul, sequence);
