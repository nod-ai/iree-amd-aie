...
#define DEBUG_TYPE "iree-amdaie-assign-buffer-address"

namespace mlir::iree_compiler::AMDAIE {

/// Utility to get the maximum memory size of a given tile.
static FailureOr<uint32_t> getMaxMemorySize(AMDAIEDeviceModel deviceModel,
                                            AMDAIE::TileOp tile) {
...
//===----------------------------------------------------------------------===//
static LogicalResult basicAllocation(AMDAIE::TileOp tile,
                                     SetVector<AMDAIE::BufferOp> buffers,
...
FailureOr<bool> checkAndAddBufferWithAddress(
    AMDAIE::BufferOp buffer, uint32_t bankSize,
    SmallVector<BankStates> &bankStates) {
...
bool setBufferAddress(AMDAIE::BufferOp buffer, uint32_t bankSize,
                      int &startBankIndex,
                      SmallVector<BankStates> &bankStates) {
  ...
  buffer.emitError("Failed to allocate buffer: ")
      << "size: " << bufferSize << " bytes across " << numBanks << " banks.";
  return false;
}
...
LogicalResult bankAwareAllocation(AMDAIE::TileOp tile,
                                  SetVector<BufferOp> buffers,
                                  AMDAIEDeviceModel deviceModel) {
  FailureOr<uint32_t> maxDataMemorySize = getMaxMemorySize(deviceModel, tile);
  std::optional<int64_t> col = getConstantIntValue(tile.getCol());
  std::optional<int64_t> row = getConstantIntValue(tile.getRow());
 ...
 public:
  ...
  void runOnOperation() override {
    ...
    workgroupOp->walk<WalkOrder::PreOrder>([&](AMDAIE::BufferOp buffer) {
      if (!(bool(
              buffer.getOperation()->template getAttrOfType<mlir::StringAttr>(
    ...
    if (!device) {
      workgroupOp->emitOpError() << "could not find an AMDAIEDevice attribute";
