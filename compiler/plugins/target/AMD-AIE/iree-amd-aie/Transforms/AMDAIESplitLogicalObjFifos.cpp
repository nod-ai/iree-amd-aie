...
/// Utility to compute the product of the sizes after each dimension:
/// productSizes[idx] will contain the product of all sizes in [idx + 1,
/// sizes.size() - 1].
SmallVector<int64_t> getProductSizesAfterIndex(ArrayRef<int64_t> sizes) {
  SmallVector<int64_t> productSizes;
  productSizes.reserve(sizes.size());
  int64_t productSize{1};
  for (auto size : llvm::reverse(sizes)) {
    productSize *= size;
    productSizes.push_back(productSize);
  }
  std::reverse(productSizes.begin(), productSizes.end());
  return productSizes;
}
...
LogicalResult collectSplittingDims(
  ...
  for (auto [dmaOp, objFifo] : dmaObjFifoPairs) {
    ...
    if (dmaOp.getTargetObjectFifo() == objFifo) {
      ...
      SmallVector<int64_t> sourceProductSizes =
          getProductSizesAfterIndex(sourceSizes.value());
      ...
      SmallVector<int64_t> targetProductSizes =
          getProductSizesAfterIndex(targetSizes.value());
