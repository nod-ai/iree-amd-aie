...
static llvm::cl::opt<TilePassPipeline> clUseTilePipeline(
    ...
    llvm::cl::values(
        ...
        clEnumValN(TilePassPipeline::DepthwiseConvolutionDecomposePipeline,
                   "depthwise-convolution-decompose", "TODO"),
...
std::unique_ptr<Pass> createJustFailPass() {
  struct JustFailPass
      : public PassWrapper<JustFailPass, OperationPass<func::FuncOp>> {
    void runOnOperation() override {
      auto operation = getOperation();
      operation.emitError("Emitting an error from JustFailPass");
      signalPassFailure();
    }
  };
...
void addDepthwiseConvolutionDecomposePassPipeline(OpPassManager &funcPassManager,
                                                    TilingConfig &tilingConfig) {

  // For now, it works when these are the same:
  addConvDecomposePassPipeline(funcPassManager, tilingConfig);
}
...
//   }
// 
//   // Promote output to local memory
//   {
//     AMDAIEBufferizeToAllocationOptions bufferizeOptions;
//     bufferizeOptions.memorySpace = 2;
//     bufferizeOptions.bufferizeOperand = BufferizeOperand::Output;
...
// 
// 
//   // funcPassManager.addPass(createJustFailPass());
// }
