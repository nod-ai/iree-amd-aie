...
struct HalfDmaCpyNdToNpuConverter final
  ...
  HalfDmaCpyNdToNpuConverter(MLIRContext *context,
    ...
    addressingGranularityInBits = deviceModel.getAddressGranularityInBits();
  }
  ...
  FailureOr<AMDAIE::NpuPushToQueueOp> insertWriteBdOps(
    ...
    for (auto iter : llvm::enumerate(llvm::zip(sizes, strides))) {
      ...
      if (iter.index() == 0 && (sizes.size() == numAddrDim || stride == 0)) {
        if (stride == 0) {
          ...
          iterationStride =
              std::max(stride * elemWidthInBits / addressingGranularityInBits,
                       (int64_t)1);
          iterationSize = size;
          if (stride == 1)
            size = (size * elemWidthInBits) / addressingGranularityInBits;
    ...
    // Default 1 for stride, as zero is not supported.
    staticStrides.insert(staticStrides.begin(),
                         numIntraAddrDim - staticStrides.size(), 1);
    ...
    uint32_t bufferLengthInWords =
        bufferLength * elemWidthInBits / addressingGranularityInBits;
    uint32_t innerBufferLength = bufferLengthInWords / repeatCount;
    uint32_t bufferOffsetInBytes = bufferOffset * elemWidthInBits / 8;

    // TODO(jornt): offset set to zero for shim as the offset is embedded in the
    // address patch.
 ...
 private:
  const AMDAIE::AMDAIEDeviceModel &deviceModel;
  uint8_t addressingGranularityInBits;
};
...
void AMDAIEControlCodeLoweringPass::runOnOperation() {
  ...
  // patterns.insert<DmaWaitOpConverter>(context);
