// RUN: iree-opt --split-input-file --pass-pipeline='builtin.module(hal.executable(hal.executable.variant(iree-amdaie-lowering-strategy{use-pass-pipeline=pad-pack})))' %s | FileCheck %s --check-prefix=CHECK-PAD-PACK
// RUN: iree-opt --split-input-file --pass-pipeline='builtin.module(hal.executable(hal.executable.variant(iree-amdaie-lowering-strategy{use-pass-pipeline=pack-peel})))' %s | FileCheck %s --check-prefix=CHECK-PACK-PEEL
