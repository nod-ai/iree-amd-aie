...
// RUN: aie_elf_files_gen_test %s "%{t:stem}dir"
// RUN: iree-opt --pass-pipeline="builtin.module(iree-amdaie-convert-device-to-control-packets{path-to-elfs="%{t:stem}dir" broadcast-core-config=false})" %s | FileCheck %s
// RUN: iree-opt --pass-pipeline="builtin.module(iree-amdaie-convert-device-to-control-packets{path-to-elfs="%{t:stem}dir" broadcast-core-config=true})" %s | FileCheck %s --check-prefix=BROADCAST
