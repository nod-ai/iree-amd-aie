...
// CHECK:             %[[PLAYER_A:.*]] = aie.dma(S2MM, 0) {sym_name = "player_a"} [{
// CHECK:               aie.use_lock(%[[LOCK_Y]], Acquire, 0)
// CHECK:               aie.dma_bd(%[[DOUBLE_BUFFER]] : memref<32xi32>, 0) {bd_id = 5 : i32, next_bd_id = 4 : i32}
// CHECK:               aie.use_lock(%[[LOCK_Y]], Release, 0)
// CHECK:             }, {
// CHECK:               aie.use_lock(%[[LOCK_X]], Acquire, 1)
// CHECK:               aie.dma_bd(%[[DOUBLE_BUFFER]] : memref<32xi32>) {bd_id = 4 : i32, next_bd_id = 3 : i32}
// CHECK:               aie.use_lock(%[[LOCK_X]], Release, -1)
// CHECK:             }, {
// CHECK:               aie.use_lock(%[[LOCK_Y]], Acquire) {acq_en = false}
// CHECK:               aie.dma_bd(%[[DOUBLE_BUFFER]] : memref<32xi32>) {bd_id = 3 : i32, next_bd_id = 5 : i32}
// CHECK:               aie.use_lock(%[[LOCK_Y]], Release, 1)
// CHECK:             }]
// CHECK:             %[[PLAYER_B:.*]] = aie.dma(S2MM, 1) {sym_name = "player_b"} [{
// CHECK:               aie.use_lock(%[[LOCK_Y]], Acquire, 1)
// CHECK:               aie.dma_bd(%[[DOUBLE_BUFFER]] : memref<32xi32>, 0) {bd_id = 2 : i32, next_bd_id = 1 : i32}
// CHECK:               aie.use_lock(%[[LOCK_Y]], Release, 0)
// CHECK:             }, {
// CHECK:               aie.use_lock(%[[LOCK_X]], Acquire, 1)
// CHECK:               aie.dma_bd(%[[DOUBLE_BUFFER]] : memref<32xi32>) {bd_id = 1 : i32, next_bd_id = 0 : i32}
// CHECK:               aie.use_lock(%[[LOCK_X]], Release, -1)
// CHECK:             }, {
// CHECK:               aie.use_lock(%[[LOCK_Y]], Acquire) {acq_en = false}
// CHECK:               aie.dma_bd(%[[DOUBLE_BUFFER]] : memref<32xi32>) {bd_id = 0 : i32, next_bd_id = 2 : i32}
// CHECK:               aie.use_lock(%[[LOCK_Y]], Release, -1)
// CHECK:             }]
// CHECK:             aie.end
// CHECK:           }
