...
extractMACOperandsFromAddOperands(Value addLhs, Value addRhs) {
  ...
  if (mulOp) return std::make_tuple(mulOp.getLhs(), mulOp.getRhs(), acc);
...
// types.
static std::optional<Value> convertValueToTargetTypeAIE2(
    ConversionPatternRewriter &rewriter, Location loc, Value inputVal,
    VectorType tgtType) {
  ...
  unsigned tgtLaneSize = aievec::getVectorLaneSize(tgtType);

  if (srcType == tgtType) return inputVal;
  ...
  if ((srcElemType == tgtElemType) && (srcLaneSize != tgtLaneSize)) {
    ...
    if (srcBitWidth == 8 && tgtBitWidth == 32 && srcLaneSize == 16) {
      ...
      auto concatOutType = aievec::createVectorType(32, srcElemType);
      auto concatOp = rewriter.create<aievec::ConcatOp>(
          loc, concatOutType, SmallVector<Value>({inputVal, inputVal}));
      auto accType = aievec::getVectorOpDestType(concatOutType, /*AIE2 =*/true);
      auto upsOp =
          rewriter.create<aievec::UPSOp>(loc, accType, concatOp.getResult());
      auto castType = aievec::createVectorType(32, tgtElemType);
      auto castOp = rewriter.create<aievec::CastOp>(
...
static SmallVector<NamedAttribute> buildAttributeListForRotationSelectOp(
    PatternRewriter &rewriter, VectorType vTy, int64_t rotation) {
  unsigned width = 0;
  auto elemTy = vTy.getElementType();
  if (auto intTy = dyn_cast<IntegerType>(elemTy)) width = intTy.getWidth();
    ...
    case 16:
      ...
      return SmallVector<NamedAttribute, 11>(
          {{fmaOp.getXstartAttrName(), StringAttr::get(ctx, "0")},
           ...
           {fmaOp.getZstepAttrName(),
            StringAttr::get(ctx, std::to_string(step))},
...
static arith::CmpIPredicate convertToIntegerPredicate(
...
struct FoldVectorExtractAndBroadcastToAIEBroadcast
  ...
  LogicalResult matchAndRewrite(
      vector::BroadcastOp bcastOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    auto extOp =
        dyn_cast<vector::ExtractOp>(adaptor.getSource().getDefiningOp());

    if (!extOp) return failure();
    ...
    if (unsigned laneSize = aievec::getVectorLaneSize(resultType);
      ...
      VectorType aievecBcastType =
          aievec::createVectorType(512 / elWidth, resultType.getElementType());
      auto concatOp = rewriter.create<aievec::ConcatOp>(
...
struct ConvertBroadcastToAIEBroadcast
  ...
  LogicalResult matchAndRewrite(
    ...
    if (laneSize * elWidth == 256) {
      VectorType vecType = aievec::createVectorType(512 / elWidth, scalarType);
...
struct ConvertMulAddToAIEVecFMAElemOpPattern
  ...
  LogicalResult matchAndRewrite(
      arith::AddIOp addOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    ...
    Type accType = aievec::getVectorOpDestType(cast<VectorType>(acc.getType()),
                                               /*AIE2 =*/true);
...
struct ConvertMulFToAIEVecMulElemOpPattern
  ...
  LogicalResult matchAndRewrite(
      arith::MulFOp mulOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    // Verify it's a vector operation
    ...
    if (rBitWidth > lBitWidth) {
      accType = aievec::getVectorOpDestType(rSrcType, /*AIE2 =*/true);
    }
    ...
    VectorType targetInputType =
        aievec::createVectorType(512 / lBitWidth, lSrcType.getElementType());
    if (rBitWidth > lBitWidth)
      targetInputType =
          aievec::createVectorType(512 / rBitWidth, rSrcType.getElementType());
    ...
    if (!lValConverted || !rValConverted) return failure();
...
struct ConvertMulIToAIEVecMulElemOpPattern
  ...
  LogicalResult matchAndRewrite(
      arith::MulIOp mulOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    // Verify it's a vector operation
    auto resultType = dyn_cast<VectorType>(mulOp.getType());
    if (!resultType) return failure();
...
struct FoldBroadcastToFMAOp : OpConversionPattern<aievec::FMAOp> {
  ...
  LogicalResult matchAndRewrite(
      aievec::FMAOp fmaOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    auto concatOp =
    ...
    if (concatDefOp) bcastOp = dyn_cast<vector::BroadcastOp>(concatDefOp);
...
struct ConvertMulAddToAIEVecFMAOpPattern : OpConversionPattern<aievec::AddOp> {
  ...
  LogicalResult matchAndRewrite(
      aievec::AddOp addOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
...
struct LowerVectorTransferReadToAIEUPD
  ...
  LogicalResult matchAndRewrite(
    ...
    if (aievec::getTransferReadAlignmentOffset(adaptor, vType, vectorAlignment)
            .value_or(0) != 0)
      return failure();
...
struct LowerVectorAddIOpToAIEVecAddOp : OpConversionPattern<arith::AddIOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    if (!isa<VectorType>(resType)) return failure();
...
struct LowerVectorMulIOpToAIEVecMulOp : OpConversionPattern<arith::MulIOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    auto accTy = aievec::getVectorOpDestType(resTy, /*AIE2 =*/false);
...
struct LowerVectorReductionMinOp : OpConversionPattern<vector::ReductionOp> {
  ...
  LogicalResult matchAndRewrite(
      vector::ReductionOp srcOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
...
struct LowerVectorReductionAddIntOp : OpConversionPattern<vector::ReductionOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    if (laneSize == 32 && elWidth == 32) {
      Location loc = srcOp.getLoc();
      VectorType vecType = aievec::createVectorType(laneSize / 2, scalarType);
...
struct LowerVectorReductionAddBfloat16Op
  ...
  LogicalResult matchAndRewrite(
    ...
    VectorType vecType = aievec::createVectorType(2 * laneSize, scalarType);
...
struct LowerVectorExtractStridedSliceOpAIEv1Pattern
  ...
  LogicalResult matchAndRewrite(
    ...
    if (aievec::getElementSizeInBits(vType) == 8)
...
struct LowerVectorExtractStridedSliceOpAIE2Pattern
  ...
  LogicalResult matchAndRewrite(
    ...
    auto topHalf = rewriter
                       ...
                       .getResult();
    int64_t offset = cast<IntegerAttr>(adaptor.getOffsets()[0]).getInt();
    int32_t shiftBytes = offset * aievec::getElementSizeInBits(vType) / 8;
...
struct ExpandUPDToUPDAndExtPattern : OpConversionPattern<aievec::UPDOp> {
  ...
  LogicalResult matchAndRewrite(
      aievec::UPDOp updOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
...
struct ComputeExpOpByLUTPattern : OpConversionPattern<math::ExpOp> {
  ...
  LogicalResult matchAndRewrite(
      math::ExpOp expOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    ...
    Type accTypeNative = aievec::getVectorOpDestType(srcType, /*AIE2 =*/true);
...
struct ComputeInvOpByLUTPattern : OpConversionPattern<arith::DivFOp> {
  ...
  LogicalResult matchAndRewrite(
      arith::DivFOp divOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    ...
    if (!isNarrowingOp(*divOp->getUsers().begin())) return failure();

    auto fType = cast<FloatType>(srcType);
    if (fType.getWidth() != 32) return failure();
...
struct ComputeTanhOpByLUTPattern : OpConversionPattern<math::TanhOp> {
  ...
  LogicalResult matchAndRewrite(
      math::TanhOp tanhOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    auto srcType = dyn_cast<VectorType>(tanhOp.getOperand().getType());
    if (!srcType) return failure();

    Type scalarType = srcType.getElementType();
    if (!isa<FloatType>(scalarType)) return failure();

    unsigned laneSize = aievec::getVectorLaneSize(srcType);
    unsigned elWidth = scalarType.getIntOrFloatBitWidth();
    if (elWidth != 16 || laneSize != 16) return failure();
...
struct ComputeSqrtOpPattern : OpConversionPattern<math::SqrtOp> {
  ...
  LogicalResult matchAndRewrite(
      math::SqrtOp sqrtOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    ...
    unsigned laneSize = aievec::getVectorLaneSize(srcType);
    unsigned elWidth = scalarType.getIntOrFloatBitWidth();
    if (elWidth != 16 || (laneSize != 16 && laneSize != 32)) return failure();
...
struct ComputeRsqrtOpPattern : OpConversionPattern<math::RsqrtOp> {
  ...
  LogicalResult matchAndRewrite(
      math::RsqrtOp rsqrtOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
...
struct ComputeErfOpPattern : OpConversionPattern<math::ErfOp> {
  ...
  LogicalResult matchAndRewrite(
      math::ErfOp erfOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
...
struct ComputeAbsOpPattern : OpConversionPattern<SrcOpTy> {
  ...
  LogicalResult matchAndRewrite(
      SrcOpTy absOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    ...
    unsigned laneSize = aievec::getVectorLaneSize(vecTy);
...
struct ComputeSigmoidOpPattern : OpConversionPattern<arith::DivFOp> {
  ...
  LogicalResult matchAndRewrite(
      arith::DivFOp divfOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    ...
    if (!hasSigmoidComputationChain(adaptor, negOp)) return failure();
...
struct ComputeCeilOpPattern : OpConversionPattern<math::CeilOp> {
  ...
  LogicalResult matchAndRewrite(
      math::CeilOp ceilOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
...
struct ComputeFloorOpPattern : OpConversionPattern<math::FloorOp> {
  ...
  LogicalResult matchAndRewrite(
      math::FloorOp floorOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
...
struct ComputeNegOpPattern : OpConversionPattern<arith::NegFOp> {
  ...
  LogicalResult matchAndRewrite(
      arith::NegFOp negOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    ...
    if (unsigned laneSize = aievec::getVectorLaneSize(srcType); laneSize != 16)
      return failure();
...
static bool hasConstNegOneValue(arith::ConstantOp constOp, unsigned elWidth) {
  ...
  if (elWidth == 32) return cstDense.getSplatValue<int32_t>() == -1;
  if (elWidth == 16) return cstDense.getSplatValue<int16_t>() == -1;
  if (elWidth == 8) return cstDense.getSplatValue<int8_t>() == -1;
  return false;
}
...
struct ComputeBxorAndBnegOpPattern : OpConversionPattern<arith::XOrIOp> {
  ...
  LogicalResult matchAndRewrite(
      arith::XOrIOp xorOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    auto srcType = dyn_cast<VectorType>(xorOp.getLhs().getType());
    if (!srcType) return failure();

    Type scalarType = srcType.getElementType();
    if (!isa<IntegerType>(scalarType)) return failure();
...
struct ComputeSignedIntRightShiftOpPattern
  ...
  LogicalResult matchAndRewrite(
      arith::ShRSIOp rsOp, OpAdaptor adaptor,
      ConversionPatternRewriter &rewriter) const override {
    auto srcType = dyn_cast<VectorType>(adaptor.getLhs().getType());
    if (!srcType) return failure();
    ...
    if (elWidth == 8) {
      VectorType halfSrcType =
          aievec::createVectorType(laneSize / 2, scalarType);
      auto rsOpLow =
      ...
      Type accType = aievec::getVectorOpDestType(halfSrcType, /*AIE2 =*/true);
      auto upsOpLow =
      ...
      Type accType = aievec::getVectorOpDestType(srcType, /*AIE2 =*/true);
      auto upsOp =
...
static void configureAIEVecV1Legalizations(ConversionTarget &target) {
  ...
  target.addDynamicallyLegalOp<aievec::FMAOp>(
      [](mlir::iree_compiler::aievec::FMAOp op) {
