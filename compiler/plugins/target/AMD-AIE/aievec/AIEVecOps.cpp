...
LogicalResult UPDOp::verify() {
  ...
  if (!sourceType) return emitError("requires memref type");
...
ParseResult UPDOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (hasVector.succeeded() && parser.parseOperand(vector)) return failure();
  ...
  if (!memrefType) return parser.emitError(typesLoc, "requires memref type");
...
ParseResult BroadcastOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (!vecType) return parser.emitError(typesLoc, "requires vector type");

  VectorType resType = llvm::dyn_cast<VectorType>(types[1]);
  if (!resType) return parser.emitError(typesLoc, "requires vector type");

  // Populate the source in result
  if (parser.resolveOperand(source, vecType, result.operands)) return failure();
...
static void printMulFMAOp(OpAsmPrinter &p, T op) {
  ...
  for (int idx = 0; idx < 2; ++idx) {
    ...
    if (op.getStep(idx).empty()) elidedAttrs.push_back(op.getStepAttrName(idx));
...
ParseResult parseAddSubOp(OpAsmParser &parser, OperationState &result) {
  ...
  if (!resultType) return parser.emitError(typesLoc, "requires vector type");
...
LogicalResult ConcatOp::verify() {
  ...
  for (auto source : srcs) {
    ...
    if (!type) return emitError("requires vector type");
    if (type != sourceType) return emitError("All sources must have same type");
  }
...
ParseResult ConcatOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  if (parser.parseOperandList(sources)) return failure();
...
LogicalResult ConcatOp::inferReturnTypes(
    MLIRContext *, std::optional<Location>, ConcatOp::Adaptor adaptor,
...
void aievec::SelectOp::print(OpAsmPrinter &p) {
  ...
  if (getYbuff()) p << ", " << getYbuff();

  // Print the attributes, but don't print attributes that are empty strings
  SmallVector<StringRef, 10> elidedAttrs;
  for (int idx = 0; idx < 2; ++idx) {
    if (getStart(idx).empty()) elidedAttrs.push_back(getStartAttrName(idx));
    if (getOffset(idx).empty()) elidedAttrs.push_back(getOffsetAttrName(idx));
    if (getOffsetHi(idx).empty())
      elidedAttrs.push_back(getOffsetHiAttrName(idx));
    if (getSquare(idx).empty()) elidedAttrs.push_back(getSquareAttrName(idx));
  }
  p.printOptionalAttrDict((*this)->getAttrs(), elidedAttrs);

  // And now print the types
  p << " : " << getXbuff().getType();
  if (getYbuff()) p << ", " << getYbuff().getType();
  p << ", " << getResult().getType();
}
...
LogicalResult aievec::SelectOp::verify() {
  ...
  if (!resultType || !xbuffType) return emitError("requires vector type");

  // The underlying scalar element type of all vectors must match
  Type rtype = resultType.getElementType();
  Type xtype = xbuffType.getElementType();
  if (rtype != xtype) return emitError("types of result and xbuff must match");
...
ParseResult SelectOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  // Parse xbuff
  if (parser.parseOperand(xbuff)) return failure();

  // Parse optional ybuff
  ParseResult hasYbuff = parser.parseOptionalComma();
  if (hasYbuff.succeeded() && parser.parseOperand(ybuff)) return failure();
  ...
  if (!xbuffType) return parser.emitError(typesLoc, "requires vector type");
  VectorType ybuffType;
  if (hasYbuff.succeeded()) {
    ybuffType = llvm::dyn_cast<VectorType>(types[1]);
    if (!ybuffType) return parser.emitError(typesLoc, "requires vector type");
  }
...
LogicalResult verifyPackUnpackOp(T op) {
  ...
  auto resultType = llvm::dyn_cast<VectorType>(op.getResult().getType());
  if (!sourceType || !resultType) return op.emitError("requires vector type");
  ...
  if (isa<PackOp>(op)) {
    ...
    if (stypeWidth != 16) return op.emitError("input must be an int16 vector");
    if (rtypeWidth != 8) return op.emitError("output must be an int8 vector");
  } else {
    if (stypeWidth != 8) return op.emitError("input must be an int8 vector");
    if (rtypeWidth != 16) return op.emitError("output must be an int16 vector");
  }
...
LogicalResult ExtElemOp::verify() {
  ...
  if (!sourceType) return emitError("source requires vector type");
