...
def get_test_shapes(shapes_id: ShapesId):
    ...
    #    latency.
    if shapes_id == ShapesId.SMALL1:
        return [
            TestShape(m=8, k=16, n=32, accumulate=False),
        ]
    if shapes_id == ShapesId.SMALL2:
        return [
            TestShape(m=8, k=16, n=16, accumulate=False),
        ]
    if shapes_id == ShapesId.SMALL3:
        return [
            TestShape(m=32, k=32, n=16, accumulate=False),
        ]
    if shapes_id == ShapesId.LARGE1:
        return [
            TestShape(m=8, k=32, n=256, accumulate=False),
        ]
