  ...
  for (iree_host_size_t i = 0; i < command_buffer->set_count; ++i) {
    ...
    for (iree_host_size_t j = 0;
         j < command_buffer->descriptor_sets[i].binding_count; ++j) {
           // these are sub buffers allocated on the heap and hence need to be deleted
           try {
...
static iree_status_t iree_hal_xrt_direct_command_buffer_discard_buffer(
  ...
  // This is not yet supported in current XRT API
  return iree_ok_status();
}
...
static iree_status_t iree_hal_xrt_direct_command_buffer_dispatch(
  ...
  // collect all the other bindings in a list
  std::vector<xrt::bo*> args_buffer;
  ...
  for (iree_host_size_t i = 0; i < set_count; ++i) {
    ...
    for (iree_host_size_t j = 0; j < binding_count; ++j) {
      xrt::bo* arg_buffer = command_buffer->descriptor_sets[i].bindings[j];
      args_buffer.push_back(arg_buffer);
  ...
  instr.sync(XCL_BO_SYNC_BO_FROM_DEVICE);

  for(auto arg_buffer : args_buffer){
    arg_buffer->sync(XCL_BO_SYNC_BO_FROM_DEVICE);
  }
