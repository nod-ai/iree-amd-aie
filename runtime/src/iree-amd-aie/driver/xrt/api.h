...
typedef enum iree_hal_xrt_command_dispatch_type_e {
  // Dispatch commands in command buffer in parallel.
  IREE_HAL_XRT_COMMAND_DISPATCH_TYPE_CONCURRENT = 0,
  // Dispatch commands in command buffer sequentially.
  IREE_HAL_XRT_COMMAND_DISPATCH_TYPE_SERIAL = 1,
} iree_hal_xrt_command_dispatch_type_t;
...
typedef struct iree_hal_xrt_device_params_t {
  ...
  iree_hal_xrt_command_dispatch_type_t command_dispatch_type;
} iree_hal_xrt_device_params_t;
