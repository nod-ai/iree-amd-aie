...
// Cast utilities between Metal id<MTLDevice> and IREE opaque
...
at offset 0. iree_hal_resource_t resource;

  iree_allocator_t host_allocator;

  // Identifier used for the driver in the IREE driver registry. We allow
overriding so that
  // multiple Metal versions can be exposed in the same process.
  iree_string_view_t identifier;

  // The list of GPUs available when creating the driver. We retain them here to
make sure
...
/*static iree_hal_xrt_driver_t* iree_hal_xrt_driver_cast(iree_hal_driver_t*
base_value) { IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_xrt_driver_vtable);
...
IREE_API_EXPORT iree_status_t iree_hal_xrt_driver_create(
  ...
  iree_status_t status =
      iree_hal_xrt_driver_create_internal(identifier, host_allocator,
  out_driver);
