...
fence::fence(const device& device, shared_handle::export_handle ehdl)
    : m_pdev(device.get_pdev()),
...
std::unique_ptr<shared_handle> fence::share() const {
...
void fence::wait(uint32_t timeout_ms) const {
