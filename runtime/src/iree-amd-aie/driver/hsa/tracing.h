...
//   ...
//   iree_hal_hsa_tracing_context_collect(queue->tracing_context);
//
...
typedef struct iree_hal_hsa_tracing_context_t iree_hal_hsa_tracing_context_t;
...
void iree_hal_hsa_tracing_context_free(iree_hal_hsa_tracing_context_t* context);
...
#define IREE_HIP_TRACE_ZONE_BEGIN(context, stream)                        \
  static const iree_tracing_location_t TracyConcat(                       \
      __tracy_source_location, __LINE__) = {NULL, __FUNCTION__, __FILE__, \
                                            (uint32_t)__LINE__, 0};       \
  iree_hal_hsa_tracing_zone_begin_impl(                                   \
