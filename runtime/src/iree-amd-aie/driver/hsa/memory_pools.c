...
static iree_status_t iree_hal_hsa_create_memory_pool(
    const iree_hal_hsa_dynamic_symbols_t* hsa_symbols, hipDevice_t hip_device,
    iree_hal_hsa_memory_pool_params_t params,
  ...
  *out_pool = NULL;
  ...
  IREE_HIP_RETURN_IF_ERROR(hsa_symbols, hipMemPoolCreate(&pool, &pool_props),
                           "hipMemPoolCreate");
  ...
  if (iree_status_is_ok(status)) {
    *out_pool = pool;
    ...
    IREE_HIP_IGNORE_ERROR(hsa_symbols, hipMemPoolDestroy(pool));
  }
  return status;
}

iree_status_t iree_hal_hsa_memory_pools_initialize(
    const iree_hal_hsa_dynamic_symbols_t* hsa_symbols, hipDevice_t hip_device,
    const iree_hal_hsa_memory_pooling_params_t* pooling_params,
    iree_allocator_t host_allocator,
    iree_hal_hsa_memory_pools_t* IREE_RESTRICT out_pools) {
  ...
  out_pools->hsa_symbols = hsa_symbols;
  out_pools->host_allocator = host_allocator;

  iree_status_t status = iree_ok_status();

  if (iree_status_is_ok(status)) {
    status = iree_hal_hsa_create_memory_pool(hsa_symbols, hip_device,
                                             pooling_params->device_local,
                                             &out_pools->device_local);
  }

  if (iree_status_is_ok(status)) {
    status = iree_hal_hsa_create_memory_pool(
        hsa_symbols, hip_device, pooling_params->other, &out_pools->other);
  }
...
void iree_hal_hsa_memory_pools_deinitialize(
    iree_hal_hsa_memory_pools_t* pools) {
  IREE_TRACE_ZONE_BEGIN(z0);

  if (pools->device_local) {
    IREE_HIP_IGNORE_ERROR(pools->hsa_symbols,
                          hipMemPoolDestroy(pools->device_local));
    pools->device_local = NULL;
  }

  if (pools->other) {
    IREE_HIP_IGNORE_ERROR(pools->hsa_symbols, hipMemPoolDestroy(pools->other));
    pools->other = NULL;
  }
...
static void iree_hal_hsa_memory_pool_track_alloc(
    iree_hal_hsa_memory_pools_t* pools, iree_hal_buffer_t* buffer) {
  ...
  IREE_TRACE_ALLOC_NAMED(
      ...
      (void*)iree_hal_hsa_buffer_device_pointer(buffer), allocation_size);
  IREE_STATISTICS({
...
static void iree_hal_hsa_memory_pool_track_free(
    iree_hal_hsa_memory_pools_t* pools, iree_hal_buffer_t* buffer) {
  bool is_device_local = iree_all_bits_set(iree_hal_buffer_memory_type(buffer),
                                           IREE_HAL_MEMORY_TYPE_DEVICE_LOCAL);
  (void)is_device_local;
...
void iree_hal_hsa_memory_pools_merge_statistics(
    iree_hal_hsa_memory_pools_t* pools,
...
iree_status_t iree_hal_hsa_memory_pools_trim(
    iree_hal_hsa_memory_pools_t* pools,
    const iree_hal_hsa_memory_pooling_params_t* pooling_params) {
...
static void iree_hal_hsa_async_buffer_release_callback(
    void* user_data, iree_hal_buffer_t* buffer) {
  iree_hal_hsa_memory_pools_t* pools = (iree_hal_hsa_memory_pools_t*)user_data;
  IREE_TRACE_ZONE_BEGIN(z0);

  hipDeviceptr_t device_ptr = iree_hal_hsa_buffer_device_pointer(buffer);
  IREE_HIP_IGNORE_ERROR(pools->hsa_symbols, hipFree(device_ptr));
  iree_hal_hsa_memory_pool_track_free(pools, buffer);

  IREE_TRACE_ZONE_END(z0);
}

iree_status_t iree_hal_hsa_memory_pools_allocate(
    iree_hal_hsa_memory_pools_t* pools, hipStream_t stream,
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_buffer_release_callback_t release_callback = {
        .fn = iree_hal_hsa_async_buffer_release_callback,
        .user_data = pools,
    };
  ...
  if (iree_status_is_ok(status)) {
    ...
    iree_hal_hsa_memory_pool_track_alloc(pools, buffer);
    *out_buffer = buffer;
  } else if (buffer) {
    iree_hal_buffer_release(buffer);
  } else {
    IREE_HIP_IGNORE_ERROR(pools->hsa_symbols, hipFreeAsync(device_ptr, stream));
  }
...
iree_status_t iree_hal_hsa_memory_pools_deallocate(
    iree_hal_hsa_memory_pools_t* pools, hipStream_t stream,
  ...
  // it asynchronously.
  iree_status_t status = iree_ok_status();
  if (iree_hal_hsa_buffer_type(buffer) == IREE_HAL_HIP_BUFFER_TYPE_ASYNC) {
    // Try to schedule the buffer for freeing.
    hipDeviceptr_t device_ptr = iree_hal_hsa_buffer_device_pointer(buffer);
    status = IREE_HIP_RESULT_TO_STATUS(
        pools->hsa_symbols, hipFreeAsync(device_ptr, stream), "hipFreeAsync");
    if (iree_status_is_ok(status)) {
      ...
      // is destroyed.
      iree_hal_hsa_buffer_drop_release_callback(buffer);
