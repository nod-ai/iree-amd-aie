...
struct iree_hal_hsa_tracing_context_t {
...
static iree_status_t iree_hal_hsa_tracing_context_initial_calibration(
    const iree_hal_hsa_dynamic_symbols_t* symbols, hipStream_t stream,
...
iree_status_t iree_hal_hsa_tracing_context_allocate(
    const iree_hal_hsa_dynamic_symbols_t* symbols,
    iree_string_view_t queue_name, hipStream_t stream,
    iree_arena_block_pool_t* block_pool, iree_allocator_t host_allocator,
    iree_hal_hsa_tracing_context_t** out_context) {
  IREE_TRACE_ZONE_BEGIN(z0);
  ...
  *out_context = NULL;
  ...
  iree_hal_hsa_tracing_context_t* context = NULL;
  ...
  if (iree_status_is_ok(status)) {
    IREE_TRACE_ZONE_BEGIN_NAMED(
        z_event_pool, "iree_hal_hsa_tracing_context_allocate_event_pool");
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_hsa_tracing_context_initial_calibration(
  ...
  if (iree_status_is_ok(status)) {
    *out_context = context;
    ...
    iree_hal_hsa_tracing_context_free(context);
  }
  IREE_TRACE_ZONE_END(z0);
  return status;
}

void iree_hal_hsa_tracing_context_free(
    iree_hal_hsa_tracing_context_t* context) {
  if (!context) return;
  IREE_TRACE_ZONE_BEGIN(z0);

  // Always perform a collection on shutdown.
  iree_hal_hsa_tracing_context_collect(context);

  // Release all events; since collection completed they should all be unused.
  IREE_TRACE_ZONE_BEGIN_NAMED(z_event_pool,
                              "iree_hal_hsa_tracing_context_free_event_pool");
  for (iree_host_size_t i = 0; i < context->query_capacity; ++i) {
...
void iree_hal_hsa_tracing_context_collect(
    iree_hal_hsa_tracing_context_t* context) {
  if (!context) return;
...
static uint16_t iree_hal_hsa_tracing_context_insert_query(
    iree_hal_hsa_tracing_context_t* context, hipStream_t stream) {
...
void iree_hal_hsa_tracing_zone_begin_impl(
    iree_hal_hsa_tracing_context_t* context, hipStream_t stream,
    const iree_tracing_location_t* src_loc) {
  if (!context) return;
  uint16_t query_id =
      iree_hal_hsa_tracing_context_insert_query(context, stream);
  iree_tracing_gpu_zone_begin(context->id, query_id, src_loc);
}

void iree_hal_hsa_tracing_zone_begin_external_impl(
    iree_hal_hsa_tracing_context_t* context, hipStream_t stream,
...
void iree_hal_hsa_tracing_zone_end_impl(iree_hal_hsa_tracing_context_t* context,
                                        hipStream_t stream) {
  if (!context) return;
  uint16_t query_id =
      iree_hal_hsa_tracing_context_insert_query(context, stream);
