...
func.func @matmul_bias(%arg0: tensor<${M}x${K}x${TYPE1}>, %arg1: tensor<${K}x${N}x${TYPE1}>, %arg2: tensor<${N}x${TYPE2}>) -> tensor<${M}x${N}x${TYPE2}>
{
  ...
  %3 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%2, %arg2 : tensor<${M}x${N}x${TYPE2}>, tensor<${N}x${TYPE2}>) outs(%0 : tensor<${M}x${N}x${TYPE2}>) {
