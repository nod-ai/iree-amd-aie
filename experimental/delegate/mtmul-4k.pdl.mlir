...
pdl.pattern @bmm1 : benefit(1) {
  ...
  //
  // ```
  // linalg.batch_matmul ins(%lhs, %rhs : tensor<1x4096x512xbf16>,
  //   tensor<1x512x4096xbf16>) outs(%64 : tensor<1x4096x4096xf32>) ->
  //   tensor<1x4096x4096xf32>
  // ```
  
  %lhs_type = pdl.type : tensor<1x4096x512xbf16>
  %rhs_type = pdl.type : tensor<1x512x4096xbf16>
  %matmul_type = pdl.type : tensor<1x4096x4096xf32>
  %fixed_M = pdl.attribute = 4096 : i32
  %fixed_N = pdl.attribute = 4096 : i32
...
pdl.pattern @bmm2 : benefit(1) {
  ...
  //
  // ```
  // linalg.batch_matmul ins(%lhs, %rhs : tensor<1x4096x4096xbf16>,
  //   tensor<1x4096x512xbf16>) outs(%64 : tensor<1x4096x512xf32>) ->
  //   tensor<1x4096x512xf32>
  // ```
  
  %lhs_type = pdl.type : tensor<1x4096x4096xbf16>
  %rhs_type = pdl.type : tensor<1x4096x512xbf16>
  %matmul_type = pdl.type : tensor<1x4096x512xf32>
  %fixed_M = pdl.attribute = 4096 : i32
  %fixed_N = pdl.attribute = 512 : i32
  %fixed_K = pdl.attribute = 4096 : i32
